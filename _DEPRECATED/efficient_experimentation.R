...
set.seed(1234)
expCtrl <- expControl(n_var = N_VAR, mode = mode, 
                      beta_sd = 0.5 , beta_zero = 0,  # >0 indicates more than 50% purchasers (for linear:  for 5%, for nonlinear: -3.12)
                      beta_tau_sd = 0.05, tau_zero = 0, # >0 indicates positive treatment effect (for linear: 0.645 for 5%, for nonlinear: 0.7)
                      DGP="linear")
...
set.seed(1234)
expCtrl <- expControl(n_var = N_VAR, mode = mode, beta_zero = 20,  # >0 indicates more than 50% purchasers (for linear:  for 5%, for nonlinear: -3.12)
                      tau_zero =   5, # >0 indicates positive treatment effect (for linear: 0.645 for 5%, for nonlinear: 0.7)
                      DGP="nonlinear")
...
idx_targeting_model <- sample(nrow(X), size=0.2*nrow(X))
...
TAU <- TAU[-idx_targeting_model]

# Train targeting model: We assume that a company would have an existing model
#targeting_model <- glm(y~., family = binomial(link="logit"), cbind(X_targeting, "y"=Y_targeting)[W_targeting==1,] )
targeting_model <- T_Logit(X=X_targeting, y=Y_targeting, W=W_targeting, prop_score = temp$prop_score[idx_targeting_model])
...
ModelMetrics::auc(actual=Y_targeting[W_targeting==0], predicted=predict(targeting_model$model0, X_targeting[W_targeting==0,], type = "response"))
ModelMetrics::auc(actual=Y_targeting[W_targeting==1], predicted=predict(targeting_model$model1, X_targeting[W_targeting==1,], type = "response"))
...
#cor(pred[W==1], TAU[W==1], method = "spearman")
qini_score(scores = pred, Y = Y, W = W, plotit=TRUE)
...
for(i in 1:NO_EXPERIMENT_ITER){
  ...
  individual[[i]] <- do_experiment(X_iter, expControl = expCtrl, y=Y, w=W,
                                   prop_score = map_propensity(predict(targeting_model, X_iter), 
                                                               target_ratio=0.5, groups=20))
  all[[i]] <- do_experiment(X_iter, expControl = expCtrl, y=Y, w=W, prop_score = 1)
}

#### Experiment Summary statistics ####
exp_summary <- foreach(exp=list(none, balanced, individual,imbalanced, all),.combine="rbind", .inorder = TRUE)%do%{
...
row.names(exp_summary) <- c("none","balanced","supervised","imbalanced","all")
round(t(exp_summary),3)

fwrite(data.table(round(t(exp_summary),3),keep.rownames = T), 
       "../experiment_summary_real_20190616.txt")
...
ggplot(data = ate_box, aes(x=Var2, y=value)) + 
  ...
  theme(axis.text.x = element_text(size=14), 
        ...
        axis.title.y = element_text(size=16)) #+ ylim(c(-0.05,0.05))
...
model_library <- foreach(i=1:N_ITER, .combine="list", .multicombine=TRUE, .export = c("predict.tlearner"), .errorhandling = "pass", .packages = "data.table")%dopar%{
  ...
  "individual" = do_experiment(X_train, expControl = expCtrl, prop_score = map_propensity(predict(targeting_model, X_train, type="response"), target_ratio=0.5))
  )
  ...
  pred[[paste0('t-logit_',rand_scheme)]] <- unname(predict(t_logit, X_test, type='response'))
  ...
  test$supervised_propensity <- map_propensity(predict(targeting_model, X_train, type="response"), target_ratio=0.5)
  
  return(list("pred"=pred, "true"=test))
}

saveRDS(model_library, "../model_library_20190616.rds")
#model_library <- readRDS("../model_library_20190430.rds")

#### Calculate performance for each model ####
# New metric can be specified here
performance_CATE <- function(tau_score, y_true=NULL, w=NULL, prop_score_mapping=NULL, prop_score=NULL, tau_true=NULL, value){
  res <- list()
  ...
  breaks = quantile(prop_score_mapping, probs = seq(0,1,0.2))
  if(length(unique(breaks))==6 & !is.null(tau_true)){
  quantiles <- cut(prop_score_mapping, breaks = breaks, include.lowest = TRUE, right=TRUE, labels = FALSE)
  #tapply(cbind(tau_score, tau_true), quantiles, function(x) mean(abs(x[,1] - x[,2])))
  for(i in 1:max(quantiles)){
    res[[paste0("MAE_quantile_",i*20)]] <- mean(abs(tau_true[quantiles==i] - tau_score[quantiles==i]))
  }
  }
...
res <- lapply(model_library, function(ITER) lapply(ITER$pred, function(PRED){
  performance_CATE(tau_score= PRED,
                   ...
                   prop_score_mapping = ITER$true$supervised_propensity)
} ))

# Combine models into one data table per iteration
res <- lapply(res, function(x) rbindlist(x,idcol="model", fill=TRUE))#, keep.rownames="model"))
